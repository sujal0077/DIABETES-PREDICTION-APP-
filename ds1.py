...
missing_values_db = df_data.isnull().sum()

# Display missing values for each column
missing_values_db

summary_stats_db = df_data.describe()

# Display summary statistics
summary_stats_db

df_data.hist(bins=10, figsize=(14, 12), layout=(3, 3), color='#3498db')
plt.tight_layout()
plt.show()

# Set up for correlation matrix
plt.figure(figsize=(12, 8))
corr_matrix_diabetes = df_data.corr()
...
for col in cols_with_zeros:
    df_data[col] = df_data[col].replace(0, df_data[col].median())
...
df_data[features_to_normalize] = scaler.fit_transform(df_data[features_to_normalize])

# Display the first few rows after normalization
df_data.head()

correlation_matrix = df_data.corr().abs()
